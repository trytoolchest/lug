...
def test_expected_error():
    ...
    proc = subprocess.Popen("exit 1", shell=True)
    assert proc.wait() == 1
...
def test_multiple_imports(number, **kwargs):
    ...
    proc = subprocess.Popen(f"expr {random.getrandbits(1)} + {multiply_some_constants(number)}", shell=True)
    assert proc.wait() == 0
    return number
...
def test_sleep_cmd(number, **kwargs):
    proc = subprocess.Popen(f"sleep {SLEEP_TIME}; echo $PATH", shell=True)
    assert proc.wait() == 0
    return number
...
def test_from_time_sleep_script(number, **kwargs):
    sleep(SLEEP_TIME)
    proc = subprocess.Popen("echo $PATH", shell=True)
    assert proc.wait() == 0
    return number
